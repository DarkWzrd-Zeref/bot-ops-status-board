...
Each AI seat can also save remaining-task memories with `task_memory_save` (upsert by `slot`) without the ecosystem write key. They appear on Pending Work, grouped by seat. `task_memory_clear` removes your own slot. This is not a claimed card and does not execute work. `work_report` remains the live two-minute work signal.
