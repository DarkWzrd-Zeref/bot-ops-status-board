...
function buildTools() {
  ...
  return `<div class="build-toolbar"><span class="eyebrow">CAMPUS EDITOR</span><div class="mode-buttons"><button data-new-project>+ Project</button>${[["play", "Select"], ["build", "Build"], ["move", "Move"]].map(([id, label]) => `<button data-mode="${id}" class="${mode === id ? "active" : ""}">${label}</button>`).join("")}</div></div>
...
export function mountHud(root: HTMLElement) {
  host = root;
  root.innerHTML = `
    <header class="app-header"><a class="brand" href="/" aria-label="AREA 67 (the hub) home"><span class="brand-mark">67</span><span>AREA <b>67</b><small>THE HUB</small></span></a>
    ...
    <div class="world-overlay"><div class="world-heading"><span class="eyebrow">YOUR CONNECTED WORKSPACE</span><h1>AREA 67 (THE HUB)</h1><span>One campus. Four wallets. No duplicate thinking.</span></div><div id="mission-summary"></div><div class="world-controls" role="group" aria-label="Map zoom"><button data-camera="out" aria-label="Zoom out" title="Zoom out">−</button><button data-camera="home" aria-label="Reset and fit campus" title="Reset and fit campus">⌖</button><button data-camera="in" aria-label="Zoom in" title="Zoom in">+</button></div><div class="world-caption"><span>Click a station to inspect · / to find anything</span><span>Drag to pan · scroll to zoom</span></div></div>
    <nav class="map-launchers" aria-label="Hub controls"><button id="agents-launcher" aria-controls="roster" aria-expanded="false">Agents</button><button id="usage-launcher" data-ecosystem="skill-altar" aria-label="Usage, shared memory and skills" title="Account usage, shared memory and skills — no map navigation needed"><span class="launcher-wide">Usage & skills</span><span class="launcher-short">Usage</span></button><button id="planning-launcher" data-ecosystem="war-table" aria-label="War Table planning" title="Open shared planning and decisions"><span class="launcher-wide">War Table</span><span class="launcher-short">Plans</span></button><button id="tools-launcher" aria-controls="operations-panel" aria-expanded="false" title="Edit campus buildings and project workspaces">Edit map</button></nav>
...
function bind() {
  window.addEventListener("area67-navigation-open", () => {
    // Search is navigation only; retain any in-progress placement and every draft.
    operationsOpen = false; friendsOpen = false; chatOpen = false; render();
  });
