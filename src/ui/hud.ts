...
export function mountHud(root: HTMLElement) {
  ...
  root.innerHTML = `
    ...
    <div class="world-overlay"><div class="world-heading"><span class="eyebrow">YOUR CONNECTED WORKSPACE</span><h1>AREA 67</h1><span>One campus. Every possibility.</span></div><div id="mission-summary"></div><div class="world-controls" role="group" aria-label="Map zoom"><button data-camera="out" aria-label="Zoom out" title="Zoom out">−</button><button data-camera="home" aria-label="Reset and fit campus" title="Reset and fit campus">⌖</button><button data-camera="in" aria-label="Zoom in" title="Zoom in">+</button></div><div class="world-caption"><span>Click a station to inspect · / to find anything</span><span>Drag to pan · scroll to zoom</span></div></div>
