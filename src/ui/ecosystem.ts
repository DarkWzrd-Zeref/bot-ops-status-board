...
function render() {
  ...
  dialog.querySelector("#ecosystem-input-label")!.textContent = altar ? "Skill name" : selected === "vision-board" ? "Idea" : selected === "pending-work" ? "Task to pick up later" : "Advancement or decision";
  dialog.querySelector("#ecosystem-body-label")!.textContent = altar ? "What can you do? Include limits or required access." : selected === "pending-work" ? "Context, current state and the next step" : "Details, evidence or discussion points";
  ...
  const rows = altar ? ecosystem.skills.filter(s => !ownerFilter || s.owner === ownerFilter).map(s => `<article class="ecosystem-card signed-skill"><div class="board-card-meta"><span>SELF-DECLARED SKILL</span><time>${date(s.signedAt)}</time></div><h3>${esc(s.name)}</h3><p>${esc(s.description)}</p>${s.sourceUrl && safeLink(s.sourceUrl) ? `<a href="${esc(s.sourceUrl)}" target="_blank" rel="noopener noreferrer">Skill source ↗</a>` : ""}<div class="skill-signature"><span>Signed by</span><strong>${esc(s.signature)}</strong><small>Owner: ${esc(speakerLabel(s.owner))} · ${s.source === "mcp" ? "Seat key + MCP" : "Human write key"}</small></div></article>`).join("") : ecosystem.cards.filter(c => c.board === selected && (includeClosed || !["done", "archived"].includes(c.status))).map(c => {
    ...
    return `<article class="ecosystem-card"><div class="board-card-meta"><span class="card-state ${c.status}">${c.status === "claimed" ? "Picked up" : c.status}</span><time>${date(c.updatedAt)}</time></div><h3>${esc(c.title)}</h3><span class="card-project">${esc(projectName(c.projectUid))}</span><p>${esc(c.body)}</p><div class="card-owner">Added by ${esc(speakerLabel(c.createdBy))}${c.claimedBy ? `<br>Picked up by <b>${esc(speakerLabel(c.claimedBy))}</b>` : ""}<small>Last updated by ${esc(speakerLabel(c.updatedBy))}</small></div><div class="card-actions">${closed ? button("reopen", "Reopen") : `${c.board === "pending-work" && c.status === "parked" ? button("claim", "Pick up as Zeref") : ""}${c.board !== "war-table" ? button("discuss", "Bring to War Table") : ""}${c.status !== "parked" ? button("park", "Park for later") : ""}${button("complete", "Mark done")}${button("archive", "Archive")}`}</div></article>`;
  }).join("");
...
export function mountEcosystem(root: HTMLElement, chat: (uid?: string) => void) {
  ...
  dialog.querySelector("#ecosystem-discuss")!.addEventListener("click", () => { const station = runtime.buildings.find(b => b.hubId === selected); if (!station) { error("Place this station from Build → Ecosystem to open its dedicated discussion."); return; } dialog.close(); openDiscussion(station.uid); });
  ...
  dialog.querySelector<HTMLFormElement>("#ecosystem-form")!.onsubmit = async e => {
    ...
    const altar = selected === "skill-altar";
    const parsed = altar ? registrationSchema.safeParse({ name: data.get("title"), description: data.get("body"), sourceUrl: data.get("sourceUrl"), signature: data.get("signature") }) : cardSchema.safeParse({ board: selected, title: data.get("title"), body: data.get("body"), ...(data.get("projectUid") ? { projectUid: data.get("projectUid") } : {}) });
