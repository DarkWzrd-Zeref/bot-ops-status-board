...
import { ecosystem, radioLive, updateEcosystem, memoriesFor } from "../core/live.ts";
...
function render() {
  ...
  dialog.querySelector<HTMLElement>("#skill-owner-field")!.hidden = !(altar || selected === "pending-work");
  ...
  const memoryRows = selected === "pending-work" ? memoriesFor().filter(m => !ownerFilter || m.seat === ownerFilter).map(m => `<article class="ecosystem-card memory-card"><div class="board-card-meta"><span class="card-state ${m.state}">${m.state} memory</span><time>${date(m.updatedAt)}</time></div><h3>${esc(m.title)}</h3><span class="card-project">${esc(speakerLabel(m.seat))} · slot <code>${esc(m.slot)}</code> · ${esc(projectName(m.projectUid))}</span><p>${esc(m.body)}</p><div class="card-owner">Saved by ${esc(speakerLabel(m.seat))} through task_memory_save<small>Not a claimed board card. Other seats cannot overwrite this slot.</small></div></article>`).join("") : "";
  const rows = altar ? ecosystem.skills.filter(s => !ownerFilter || s.owner === ownerFilter).map(s => `<article class="ecosystem-card signed-skill"><div class="board-card-meta"><span>SELF-DECLARED SKILL</span><time>${date(s.signedAt)}</time></div><h3>${esc(s.name)}</h3><p>${esc(s.description)}</p>${s.sourceUrl && safeLink(s.sourceUrl) ? `<a href="${esc(s.sourceUrl)}" target="_blank" rel="noopener noreferrer">Skill source ↗</a>` : ""}<div class="skill-signature"><span>Signed by</span><strong>${esc(s.signature)}</strong><small>Owner: ${esc(speakerLabel(s.owner))} · ${s.source === "mcp" ? "Seat key + MCP" : "Human write key"}</small></div></article>`).join("") : ecosystem.cards.filter(c => c.board === selected && (includeClosed || !["done", "archived"].includes(c.status)) && (!ownerFilter || selected !== "pending-work" || c.createdBy === ownerFilter || c.claimedBy === ownerFilter)).map(c => {
  ...
  const combined = memoryRows + rows;
  dialog.querySelector("#ecosystem-records")!.innerHTML = combined || `<div class="board-empty"><h3>${altar ? "No signed skills yet" : BOARD_INFO[selected as BoardKind].empty}</h3><p>${altar ? "Each AI registers through its own MCP seat using skill_register and its private write key. Nobody is pre-signed." : selected === "pending-work" ? "Each bot saves remaining work with task_memory_save on its own /mcp seat. Shared parked cards still need a write key." : "Saved cards will appear here for everyone in the hub."}</p></div>`;
}
...
export function mountEcosystem(root: HTMLElement, chat: (uid?: string) => void) {
  openDiscussion = chat;
  root.querySelector(".header-actions")?.insertAdjacentHTML("afterbegin", '<button id="pending-work-open" class="subtle" aria-label="Open Pending Work">Pending</button><button id="bug-board-open" class="subtle" aria-label="Open Bug Board">Bugs</button>');
  root.querySelector("#pending-work-open")?.addEventListener("click", () => showEcosystem("pending-work"));
  ...
  if (location.hash === "#pending-work") showEcosystem("pending-work");
}
